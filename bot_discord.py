...
TOKEN = 'NzA4NDk0OTI2MzIwNTAwNzM2.XrykEA.rjaf6NBeNz2Oz_qMRZNuNnQzu8M'
...
async def on_ready():
	print ('ONLINE and READY FOR WORK')
...
async def on_raw_reaction_add(payload):
	if payload.message_id == 710604319891783681:
		guild = bot.get_guild(payload.guild_id)
		role = None

		if str(payload.emoji) == '🌈':
			role = guild.get_role(710590323608518706)
		elif str(payload.emoji) == '🐒':
			role = guild.get_role(710590486330605670)
		elif str(payload.emoji) == '🏆':
			role = guild.get_role(710590373772132525)
	...
	if payload.message_id == 710604319891783681:
		...
		if role:
			...
			if member:
				...
				print(f'Role zabrana')


@bot.command(pass_context = True,aliases=['вики']) #!!wiki  !!вики
